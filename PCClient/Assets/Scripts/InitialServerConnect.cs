        ...
        try
        {
            ...
            while (true)
            {
                ...
                using (NetworkStream stream = socketConnection.GetStream())
                {
                    ...
                    while ((length = stream.Read(bytes, 0, bytes.Length)) != 0 && string.Compare(currMessage, "") == 0)
                    {
                        ...
                        else if (string.Compare(serverMessage, "You are connected. Waiting for VR Player...") == 0)
                        {
                            Debug.Log("Waiting for VR");//Display waiting message
                        }
                        else
                        {
